...
describe("WebMCP registration", () => {
  it("registers top-level tools with read-only annotations", async () => {
    ...
    const session: ReviewSession = { version:1, sessionId:"review-1", route:"/", viewport:{width:1,height:1,devicePixelRatio:1,preset:"desktop"}, createdAt:"now", status:"ready", submittedAt:"later", summary:{ total:1, byOperation:{ move:1 } }, annotations:[], implementationInstruction:IMPLEMENTATION_INSTRUCTION };
    ...
    expect(getSession?.execute?.()).toMatchObject({ status: "ready", submittedAt: "later", summary: { total: 1 } });
  });
});
