...
describe("WebMCP registration", () => {
  it("registers top-level tools with read-only annotations", async () => {
    const definitions: Array<{ name: string; annotations?: { readOnlyHint?: boolean } }> = [];
    Object.defineProperty(document, "modelContext", { configurable: true, value: { registerTool: vi.fn((tool) => definitions.push(tool)) } });
    const session: ReviewSession = { version:1, sessionId:"review-1", route:"/", viewport:{width:1,height:1,devicePixelRatio:1,preset:"desktop"}, createdAt:"now", annotations:[], implementationInstruction:IMPLEMENTATION_INSTRUCTION };
